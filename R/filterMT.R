#' Filter a RangedSummarizedExperiment's columns based on its $mtCovg mcols 
#'
...
#' 
#' @param SE          a RangedSummarizedExperiment with an mcols() named mtCovg
...
filterMT <- function(SE, minCovg=20) {
  ...
  } else if (! "mtCovg" %in% names(mcols(SE)) ) { 
    stop("filterMT requires a RangedSummarizedExperiment with mcols 'mtCovg'.")
  }
